...
int run(
  ...
  const std::size_t splitk_workspace_bytes =
      sm120_nvfp4::splitk_experiment::workspace_size(
          1, m, n, k, split_k);
  if (splitk_workspace_bytes == 0) {
    throw std::invalid_argument("invalid Split-K configuration");
  }
  ...
  try {
    ...
    auto launch_splitk = [&] {
      check_status(
          sm120_nvfp4::splitk_experiment::launch(
              1, m, n, k, split_k,
